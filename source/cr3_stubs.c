/* cr3_stubs.c -- link-only no-op bodies for the CR3-inherited text2bitmap / movie / editbox
 * helpers that jni_fake.c's fallback handlers reference. Unity drives none of these paths; only
 * data_dir() is live (feeds the JNI data-path getters). Avoids pulling in FreeType/FFmpeg. */
...
/* Game data root (GAME_HOME == sdmc:/switch/colorsheep_nx), used by the JNI path getters. */
const char *data_dir(void) { return GAME_HOME; }

/* Engine-thread-finished hook; our main loop uses jni_quit_requested instead, so no-op. */
