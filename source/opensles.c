...
uint32_t slCreateEngine(void **pEngine, uint32_t numOptions, const void *pEngineOptions,
  ...
  /* Reaching this at all proves FMOD selected the OpenSL output and its init ran
   * (registerLib -> dlopen("libOpenSLES.so") -> dlsym("slCreateEngine")). If FMOD
   * still reports error 60 after this line appears, the failure is downstream. */
  debugPrintf("[fmod] slCreateEngine() -- FMOD OpenSL output init reached\n");
  if (!g_reg_lock)
    g_reg_lock = SDL_CreateMutex();
  if (!pEngine)
    return SL_RESULT_PARAMETER_INVALID;
